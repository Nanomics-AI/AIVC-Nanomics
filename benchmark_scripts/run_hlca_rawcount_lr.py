...
HLCA_PATH = "benchmark_data/hlca/" "688185ad-11c2-4172-a53a-f4f1f4076860.h5ad"

BENCHMARK_PATH = "benchmark_data/hlca/" "benchmark_ann_level3_all18_seed42.csv"
...
print("  HLCA adata.raw.X")
print("  raw count expression")
...
train_meta = benchmark[benchmark["split"] == "train"].copy()

test_meta = benchmark[benchmark["split"] == "test"].copy()
...
train_cell_index = train_meta["cell_index"].to_numpy(dtype=np.int64)

test_cell_index = test_meta["cell_index"].to_numpy(dtype=np.int64)


print()
print("Extracting train expression...")

X_train = X_full[train_cell_index].tocsr()


print("Extracting test expression...")

X_test = X_full[test_cell_index].tocsr()
...
y_train = train_meta["label"].astype(str).to_numpy()

y_test = test_meta["label"].astype(str).to_numpy()
...
if X_train.shape[0] != len(y_train):
    raise RuntimeError("Train X/y mismatch.")

if X_test.shape[0] != len(y_test):
    raise RuntimeError("Test X/y mismatch.")

if X_train.shape[1] != 27402:
    raise RuntimeError("Unexpected gene dimension.")


if not np.isfinite(X_train.data).all():
    raise RuntimeError("Non-finite values in train X.")

if not np.isfinite(X_test.data).all():
    raise RuntimeError("Non-finite values in test X.")
...
print(f"Accuracy          : " f"{accuracy:.6f}")

print(f"Macro-F1          : " f"{macro_f1:.6f}")

print(f"Balanced Accuracy : " f"{balanced_acc:.6f}")
...
suffix = (
    "full"
    if (args.max_train == 0 and args.max_test == 0)
    else (f"smoke_" f"{len(y_train)}train_" f"{len(y_test)}test")
)
...
summary = pd.DataFrame(
    [
        {
            "representation": "HLCA_raw_X_counts",
...
prediction_df = pd.DataFrame(
    {
        "cell_index": test_meta["cell_index"].to_numpy(),
        "true_label": y_test,
...
prediction_path = os.path.join(
    OUT_DIR,
    (f"rawcount_lr_" f"predictions_" f"{suffix}.csv"),
)
