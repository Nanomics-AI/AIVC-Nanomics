...
class Tahoe100MDataModule(L.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        if subset_manifest:
            ...
            log.info(
                f"Loading fixed training subset from: "
                f"{subset_manifest}"
            )
            ...
            selected_train_files = subset[
                "selected_training_shards"
            ]

            expected_val_files = subset.get(
                "validation_shards"
            )

            # 确保 validation 没有偷偷改变
            if (
                expected_val_files is not None
                and expected_val_files != self.val_files
            ):
                raise RuntimeError(
            ...
            unknown_files = [
                p
                for p in selected_train_files
                if p not in full_train_set
            ]

            if unknown_files:
                raise RuntimeError(
                    f"Subset manifest contains "
                    f"{len(unknown_files)} files "
                    f"outside the training pool."
                )

            if len(selected_train_files) != len(
                set(selected_train_files)
            ):
                raise RuntimeError(
                    "Duplicate training shards found "
                    "in subset manifest."
                )
    ...
    def _create_dataloader(self, file_list: List[str], *, is_train: bool) -> DataLoader:
        ...
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            log.info(
                ...
                f"using {len(file_list)}/{original_num_files} parquet files."
            )
        ...
        if is_train:
            ...
            log.info(
                f"Train shuffle: epoch={epoch}, "
                f"seed={shuffle_seed}, rank={rank}"
            )
