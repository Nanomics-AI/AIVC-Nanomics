...
try:
    ...
    print(
        "WARNING: Libs for EmbeddingQualityValidator not found. 'pip install pandas seaborn matplotlib-base umap-learn'. Callback will be disabled."
    )
...
try:
    ...
    raise ImportError(
        "This module requires `datasets` and `huggingface_hub`. Please install them."
    )
...
class JepaLightningModule(L.LightningModule):
    def __init__(
        ...
        self.num_batches_to_cache = max(
            0, int(self.hparams.exp_config.validation_num_batches)
        )
        if self.num_batches_to_cache > 0:
            log.info(
                f"JepaLightningModule configured to cache {self.num_batches_to_cache} batches for validation callbacks."
            )
        ...
        self.register_buffer(
            "teacher_dispersion_ema", torch.tensor(0.0)
        )  # EMA of dispersion metric (scale-invariant)
        self.register_buffer(
            "stable_score", torch.tensor(0.0)
        )  # patience accumulator (float)
        ...
        self.pre_stability_sim_scale = (
            0.0  # 10% of the final sim weight before stability
        )
        self.dispersion_threshold = (
            0.07  # gate opens when dispersion (1 - mean cos) > 0.10
        )
        ...
        self.safety_flip_after = max(
            1, int(0.10 * max(self.total_steps, 1))
        )  # force-open after 10% steps

        # Make sure teacher is a true copy and frozen at train start (device-safe)
        self.register_buffer(
            "did_initial_teacher_sync", torch.tensor(False, dtype=torch.bool)
        )
    ...
    def _log_tokenizer_grad_balance(self):
        ...
        try:
            ...
            if (self.global_step % 50 == 0) and self.trainer.is_global_zero:
                ...
                print(
                    f"[GRAD] identity:{id_norm:.3e} value:{val_norm:.3e} ratio:{ratio:.2f}"
                )
        except Exception:
    ...
    def _calculate_vicreg_loss(
        ...
        if batch_size < 2:
            ...
            cov_loss = (
                p_cov.pow(2).sum() - torch.diagonal(p_cov).pow(2).sum()
            ) / p.shape[1]
        ...
        if len(self._validation_cache) < self.num_batches_to_cache:
            if batch["indices"].numel() > 0:
                ...
                self._validation_cache.append(
                    {"embeddings": embeddings.cpu(), "metadata": metadata}
                )
    ...
    def configure_optimizers(self) -> Dict:
        ...
        if len(decay_params) + len(no_decay_params) == 0:
            raise RuntimeError(
                "No parameters collected for optimization. "
        ...
        try:
            ...
            if dm_gene_map:
                if isinstance(dm_gene_map, dict):
                    ...
                    ordered = [
                        sym
                        for sym, _id in sorted(
                            dm_gene_map.items(), key=lambda kv: kv[1]
                        )
                    ]
                else:
    ...
    def on_load_checkpoint(self, checkpoint: Dict[str, Any]) -> None:
        if "ema_state_dict" in checkpoint:
            ...
            log.warning(
                "Could not find 'ema_state_dict' in checkpoint. Teacher model may be randomly initialized."
            )
...
def _write_foundation_map(
    out_path: str, gene_map_or_list, global_mean=None, global_std=None
):
    ...
    if isinstance(gene_map_or_list, dict):
        items = sorted(
            ((k, int(v)) for k, v in gene_map_or_list.items()), key=lambda kv: kv[1]
        )
    ...
    if ext == ".parquet":
        _pd.DataFrame({"gene_symbol": symbols, "token_id": token_ids}).to_parquet(
...
def _build_gene_map_from_metadata(meta_path: str):
    ...
    if ext == ".parquet":
        ...
        if {"gene_symbol", "token_id"}.issubset(df.columns):
            return {
                _normalize_symbol(s): int(t)
                for s, t in zip(df["gene_symbol"], df["token_id"])
            }
            ...
            raise ValueError(
                "gene_metadata.parquet must have 'gene_symbol' (+ optional 'token_id')."
            )
    else:
        ...
        if isinstance(j, dict) and "gene_symbol" in j and "token_id" in j:
            return {
                _normalize_symbol(s): int(t)
                for s, t in zip(j["gene_symbol"], j["token_id"])
            }
...
if __name__ == "__main__":
    ...
    if cli.export_foundation_map and cli.export_only:
        if not cli.foundation_meta:
            raise SystemError(
                "Provide --foundation-meta pointing to training gene metadata (parquet/json)."
            )
    ...
    if load_dataset is None:
        log.error(
            "FATAL: Hugging Face 'datasets' library is required to run this script. Please run 'pip install datasets'."
        )
        exit(1)
    ...
    effective_batch_size = (
        data_config.batch_size * num_devices * train_config.accumulate_grad_batches
    )
    log.info(
        f"Running on {num_devices} devices. Effective batch size: {effective_batch_size} "
    ...
    log.info(
        f"Finalized model configuration: Set gene vocabulary size to {model_config.gene_vocab_size}"
    )
    ...
    steps_per_epoch = math.ceil(
        data_config.train_samples / (data_config.batch_size * num_devices)
    )
    updates_per_epoch = math.ceil(
        steps_per_epoch / train_config.accumulate_grad_batches
    )  # FIXED: ceil, not floor
    ...
    trainer_module = JepaLightningModule(
        model_config, train_config, exp_config, total_steps
    )

    # --- STEP 6: CONFIGURE THE LOGGERS ---

    # 1. Existing Weights & Biases logger
    wandb_logger = None
    ...
    if WandbLogger and wandb:
        ...
        if wandb_logger.experiment:
            wandb_logger.watch(
                ...
                log_freq=max(
                    100,
                    exp_config.log_every_n_steps * 5,
                ),
            )
        ...
        log.warning("`wandb` not found. W&B logging is disabled.")

    # 2. New TensorBoard logger
    tensorboard_logger = TensorBoardLogger(
        save_dir="logs/tensorboard",
        name=(exp_config.wandb_run_name or "GeneJEPA"),
    )

    log.info("TensorBoard logger initialized at: " f"{tensorboard_logger.log_dir}")

    # 3. Give Lightning all available loggers.
    # Keep W&B first so existing logger behavior changes as little as possible.
    loggers = []
    ...
    #                trainer_module.model,
    #                log="all",
    #                log_freq=max(100, exp_config.log_every_n_steps * 5),
    #            )
    #            log.info("Weights & Biases logger initialized and watching the model.")
    #    else:
    #        log.warning(
    #            "`wandb` not found. Logging is disabled. Advanced validation callbacks will also be disabled."
    #        )
    ...
    # Recovery-only checkpoint:
    # keep one resumable checkpoint updated every 100 optimizer steps.
    # The main validation checkpoint above is intentionally left unchanged.
    ...
    callbacks.append(recovery_checkpoint)

    log.info(
        "Recovery checkpoint enabled: saving every 100 training steps "
        f"to {recovery_dir}"
    )
    ...
    if not datamodule.gene_map:
        raise RuntimeError(
            "DataModule's `gene_map` is not initialized. Cannot create SupervisedValidatorCallback."
        )
    ...
    if wandb_logger is not None:
        log.info(
            "W&B logger detected. "
            "Adding UMAP visualization callback "
            "(EmbeddingQualityValidator)."
        )
    ...
    if num_devices > 1:
        log.info(
            "Multiple devices detected. Using DDPStrategy(find_unused_parameters=True)."
        )
    ...
    log.info(
        f"Hardware support for bfloat16: {supports_bf16}. Using precision: '{resolved_precision}'"
    )
    ...
    ckpt_path = None
    try:
        # Compare the normal end-of-epoch last.ckpt with the recovery last.ckpt.
        # Resume from whichever one was written most recently.
        main_last_ckpt = os.path.join(
            exp_config.checkpoint_dir,
            "last.ckpt",
        )

        recovery_last_ckpt = os.path.join(
            exp_config.checkpoint_dir,
            "recovery",
            "last.ckpt",
        )
        ...
        resume_candidates = [
            path
            for path in (main_last_ckpt, recovery_last_ckpt)
        ...
        if resume_candidates:
            ...
            # Fallback for older runs that may only have named checkpoints
            # in the main checkpoint directory.
            candidates = [
